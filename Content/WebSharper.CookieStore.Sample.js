...
function Main(){
  ...
  const t_1=(t.h.push(EventQ2(t.k, "getcookie", () => t.i, () => {
    const _3=null;
    Start(Delay(() => Bind(AsAsync(getCookie()), () => Return(null))), null);
  })),t);
  const t_2=(t_1.h.push(EventQ2(t_1.k, "setcookie", () => t_1.i, () => {
    const _3=null;
    Start(Delay(() => Bind(AsAsync(setCookie()), () => Return(null))), null);
  })),t_1);
  const b=(t_2.h.push(EventQ2(t_2.k, "deletecookie", () => t_2.i, () => {
    const _3=null;
    Start(Delay(() => Bind(AsAsync(deleteCookie()), () => Return(null))), null);
  })),t_2);
...
function cookieChanges(){
  cookieStore().addEventListener("change", (event) => {
    ...
    return _1("Cookie Changed: "+String(event));
  });
}
...
function getCookie(){
  ...
  return new Promise((_2) => {
    _2(cookieStore().get("username").then((d) => d==null?(statusMessage().Set("No cookie found!"),Promise.resolve(null)):(statusMessage().Set("Cookie Found: "+String(d.name)+"="+String(d.value)),Promise.resolve(null))));
  });
}
function setCookie(){
  const _1=null;
  return new Promise((_2) => {
    let r;
    _2(cookieStore().set("username", "JohnDoe", (r={},r.expires=Date.now()+86400*1000,r)).then(() => {
      statusMessage().Set("Cookie Set: username=JohnDoe");
...
function deleteCookie(){
  const _1=null;
  return new Promise((_2) => {
    _2((cookieStore())["delete"]("username").then(() => {
      statusMessage().Set("Cookie Deleted!");
      return Promise.resolve(null);
    }));
  });
}
function cookieStore(){
  return _c_7.cookieStore;
}
...
let _c_7=Lazy((_i) => class $StartupCode_Client {
  ...
  static {
    this.statusMessage=_c_11.Create_1("No cookie set.");
    this.cookieStore=globalThis.cookieStore;
  }
});
function NewGuid(){
...
function ChildrenTemplate(el, fillWith){
  ...
  return!Equals(m, null)&&m.length===1&&(get(m, 0)instanceof Node&&(Equals(get(m, 0).nodeType, Node.ELEMENT_NODE)&&(_1=get(m, 0),true)))?_c_32.TreeNode(docTreeNode, updates):_c_9.Mk(TreeDoc(docTreeNode), updates);
}
...
function InlineTemplate(el, fillWith){
  ...
  const tryGetAsDoc=(name) => {
    ...
    if(m[0]){
      ...
      else if(th instanceof _c_5)return Some(_c_9.TextNode(th.Value));
      else {
